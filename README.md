...
- 4 Python classes working in tandem:
  ...
  - `Adtech` is an abstract class that represents any adtech. On concrete class definitions, payload formatting and API specific methods are declared based on the abstractmethods provided.
...
erDiagram
    ...
    Adtech {
        AdtechAPI api "Instance of API innerclass"
        ...
        AdtechStatus[Enum] status "Audience status in the Adtech server"
        dict response "Response of the Aftech server API call"
        dict state "Dynamically updated state"
    }
    Audience ||--o{ DataSource : instantiates
    DataSource {
...
sequenceDiagram
    ...
    participant Adtech
    loop Generator
        Catalog->>Bucket: Fetch state.yml<br/>& data.parquet.gz
...
Now that the audience was created in `AdtechA`, future runs will neither request data nor upload it to again.

___

#### 3. Further Configurations

This state file can be later reconfigured to include `AdtechB`, with its own parameter `expiration_time`, set to one of the possible Enum values declared in `AdtechB.ExpirationTime`:
...
For this demo, two audience state files `demo_audience_1.yml` and `demo_audience_2.yml` are set in the [local bucket state directory](./bucket/state/); and corresponding data files `demo_audience_1.parquet.gz` and `demo_audience_2.parquet.gz` are available also in the [local bucket data directory](./bucket/data/).

Configurations for an `ApiGateway` connection (via its concrete subclass of `DataSource`) to the data source are illustrative, and no calls will be made since all data is already available.

Two concrete classes of `Adtech` were defined: `AdtechA` and `AdtechB`. These will illustrate two different data destinations for the audiences, also with no API calls made.

To better illustrate code behaviour, a [TRYME.ipynb](./dmp/TRYME.ipynb) is provided in this repo.

___

## Collaboration

Please feel free to create issues, forks and pull requests to help improve this design!
